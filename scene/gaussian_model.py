...
#        planarmasks, prior_depth = viewpoint_cam.planarmask, #viewpoint_cam.priordepth
    def plane_initdensify(self, viewpoint_cam, vis_mask, prp):
        ...
        for i in range(1, seg_num):
            ...
            plane_gspoints = gs_points[gs_seg_mask == i]  

            if plane_gspoints.shape[0] > 1:  
                # Skip when the planar-region GS density is high.
                points_distance = torch.mean(torch.sqrt(torch.clamp_min(distCUDA2(plane_gspoints), 0.0000001)))
                if points_distance / all_points_distance < prp.planeinit_threshold:  
                    continue

                # Randomly pick k points and inject them into the GS set.
                prior_indices = torch.randperm(N)[:min(prp.init_random, N)]
                new_xyz = prior_planepoints[prior_indices, :]
                gs_nearest = find_nearest(new_xyz, gs_points, gs_seg_mask == i)

            else:  # Case where there are no Gaussian points in the region
                prior_indices = torch.randperm(N)[:min(prp.init_random, N)]
                new_xyz = prior_planepoints[prior_indices, :]
            ...
            new_opacity = self._opacity[gs_nearest, :]
            new_knn_f = self._knn_f[gs_nearest, :]  
